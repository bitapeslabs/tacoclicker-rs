import { Psbt, Signer, Transaction, address, networks, payments, script, crypto } from "bitcoinjs-lib";
import { toXOnly } from "bitcoinjs-lib/src/psbt/bip371";
import { ECPairFactory, ECPairInterface } from "ecpair";
import { LEAF_VERSION_TAPSCRIPT } from "bitcoinjs-lib/src/payments/bip341";
import { tapleafHash } from "bitcoinjs-lib/src/payments/bip341";
import { randomBytes } from "crypto";
import { formatInputsToSign, getVSize, toFormattedUtxo, witnessStackToScriptWitness } from "./utils";
import { extractWithDummySigs, toEsploraTx } from "./utils";

import type { FormattedUtxo, AlkanesUtxoEntry, AlkaneId, IEsploraTransaction } from "@/apis";
import { borshSerialize } from "borsher";
import { BoxedError, type BoxedResponse, BoxedSuccess, isBoxedError } from "@/boxed";
import chalk from "chalk";
import { addInputDynamic, trimUndefined, tweakSigner } from "./utils";
import { Provider } from "@/provider";
import { encipher, encodeRunestoneProtostone, p2tr_ord_reveal, ProtoStone } from "alkanes";
...
function isBTCTransfer(transfer: SingularTransfer): transfer is SingularBTCTransfer {
...
export class ProtostoneTransactionWithInscription<T> {
  ...
  constructor(
    ...
    this.inscriptionBytes = new Uint8Array(borshSerialize(this.inscription.borshSchema, this.inscription.shape));
  }
  ...
  public async buildCommit(): Promise<Psbt> {
    ...
    const commitTransfer: SingularBTCTransfer = {
      asset: "btc",
      amount:
        546 + getVSize(Buffer.from(this.inscriptionBytes!)) * (this.opts.feeRate ?? this.opts.provider.defaultFeeRate),
  ...
  public finalizeCommit(txHex: string) {
    ...
    tx.outs.forEach((output, index) => {
      this.utxoTweak.add.push(toFormattedUtxo(this.signedCommitEsploraTx!, txHex, this.changeAddress, index));
    });
  }
  ...
  public async buildReveal(): Promise<Psbt> {
    ...
    const commitUtxo = this.utxoTweak.add.find(
      (u) => u.txId === this.signedCommitTx!.getId() && u.scriptPk === expectedScriptPk,
    );

    if (!commitUtxo) {
      throw new Error(`Commit UTXO not found in utxoTweak. Expected scriptPk: ${expectedScriptPk}`);
    }
  ...
  public extractPsbts(): [string, string] {
    if (!this.commitPsbt || !this.revealPsbt) throw new Error("Build both PSBTs first");
  ...
  */
  ...
  constructor(
    ...
    this.transactionOptions = {
      ...
      ignoreAlkanesRequirementCheck: options.ignoreAlkanesRequirementCheck ?? false,
  ...
  private async fetchResources(): Promise<void> {
    this.availableUtxos = consumeOrThrow(await this.sandshrew_getFormattedUtxosForAddress(this.changeAddress)).filter(
      (utxo) => !this.transactionOptions.availableUtxoTweak?.remove!.has(`${utxo.txId}:${utxo.outputIndex}`),
    );

    this.transactionOptions.availableUtxoTweak!.add!.forEach((utxo) => {
      const utxoId = `${utxo.txId}:${utxo.outputIndex}`;

      if (this.availableUtxos.some((u) => `${u.txId}:${u.outputIndex}` === utxoId)) {
  ...
  private async calculateFee(): Promise<void> {
    ...
    if (!feeRate) {
      const feeResp = this.transactionOptions.feeOpts ? await this.esplora_getfee() : new BoxedSuccess(1);
      feeRate = isBoxedError(feeResp) ? 1 : feeResp.data;
    }
    //Seee suggestion @ https://github.com/bitcoinjs/bitcoinjs-lib/issues/1566
    const baseFee =
      Math.ceil(
        (this.transactionOptions.feeOpts?.vsize ?? 0) + (this.transactionOptions.feeOpts?.input_length ?? 0) * 2,
      ) * feeRate;
  ...
  private isUnlockedUtxo(utxo: FormattedUtxo): boolean {
    let isUnlocked = utxo.inscriptions.length === 0 && Object.keys(utxo.runes).length === 0;
    if (!isUnlocked) {
      console.warn(
        chalk.yellow(`UTXO ${utxo.txId}:${utxo.outputIndex} is locked due to inscriptions or runes. Skipping...`),
      );
    }
  ...
  private getAlkanesUtxosToMeetAlkanesRequirement(): Set<FormattedUtxo> {
    ...
    if (this.transactionOptions.overrideInputs || this.transactionOptions.ignoreAlkanesRequirementCheck) {
      return alkanesUtxos;
    }
    ...
    for (const alkanes of Object.keys(this.cumulativeSpendRequirementAlkanes)) {
      ...
      */
      ...
      if (accumulated < this.cumulativeSpendRequirementAlkanes[alkanes]) {
        throw new Error(`Insufficient Alkanes UTXOs to meet the requirement for ${alkanes}.`);
      }
    }
    return alkanesUtxos;
  }
  ...
  private getEsploraUtxosToMeetAllRequirements(): FormattedUtxo[] {
    ...
    const sortedUtxos = [...this.availableUtxos].sort((a, b) => b.satoshis - a.satoshis);
  ...
  private getBtcOutputs(): Record<string, number> {
    ...
    for (const transfer of this.transactionOptions.transfers) {
      ...
      const current = cumulativeBtcRequirementsPerAddress[transfer.address] || 0;

      if (isBTCTransfer(transfer)) {
        cumulativeBtcRequirementsPerAddress[transfer.address] = current + transfer.amount;
      } else {
    ...
    for (const address in cumulativeBtcRequirementsPerAddress) {
      ...
      cumulativeBtcRequirementsPerAddress[address] = Math.max(amount, this.MINIMUM_PROTOCOL_DUST);
    }
  ...
  private addOutputs(btcOutputs: Record<string, number>): boolean {
    ...
    const totalOutputValue = Object.values(btcOutputs).reduce((acc, amount) => acc + amount, 0);

    const totalInputValue = this.utxos.reduce((acc, utxo) => acc + utxo.satoshis, 0);

    const changeValue = Math.round(
      totalInputValue - totalOutputValue - this.fee - this.cumulativeValueInPsbts - this.MINIMUM_PROTOCOL_DUST,
    );
  ...
  private addInputs(): void {
    ...
    for (const input of this.transactionOptions.includeInputs ?? []) {
      ...
      addedInputs.add(`${input.input_formatted.txId}:${input.input_formatted.outputIndex}`);
    }
  ...
  private createEdicts(btcOutputs: Record<string, number>, hasChange: boolean): IEdict[] {
    ...
    for (const transfer of this.transactionOptions.transfers) {
      ...
      const alkanesEdict = {
        id: new ProtoruneRuneId(u128(transfer.asset.block), u128(transfer.asset.tx)),
      ...
      if (!alkanesEdicts[address][alkanesId]) {
        ...
        alkanesEdicts[address][alkanesId].amount = u128(
          BigInt(alkanesEdicts[address][alkanesId].amount) + BigInt(alkanesEdict.amount),
        );
      }
    }

    const transactionEdicts = Object.values(alkanesEdicts).flatMap((addressEdicts) =>
  ...
  private appendPsbtsToTx(): void {
    ...
    for (const sellerBase64 of this.transactionOptions.includePsbts!) {
      try {
        ...
        console.error(chalk.red("Failed to append PSBT to transaction:"), error);
...
export type ProtostoneTransactionOptions = (typeof ProtostoneTransaction)["prototype"]["transactionOptions"];
...
export async function getDummyProtostoneTransaction(
  ...
  try {
    ...
    return new BoxedError(
      "TransactionError",
      "Failed to create dummy transaction: " + (e instanceof Error ? e.message : "Unknown error"),
    );
  }
}
...
export async function getProtostoneUnsignedPsbtBase64(
  ...
  >
> {
  const response = await getDummyProtostoneTransaction(addressProvided, options);
...
export async function getProtostoneTransactionsWithInscription<T>(
  ...
  try {
    const wrapper = new ProtostoneTransactionWithInscription(addressProvided, inscription, options);
    ...
    console.error("Error creating Protostone transactions with inscription:", error);
    return new BoxedError(
      "TransactionError",
